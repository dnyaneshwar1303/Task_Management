...
export default function AdminDashboard({ teams = [], users = [], onTaskUpdated }) {
  const { user } = useAuth();
  const [tasks, setTasks] = useState([]);
  const [stats, setStats] = useState({ totalTasks: 0, todoCount: 0, inProgressCount: 0, doneCount: 0, totalTeams: 0 });
  ...
  // Admin Capability: Assign tasks to managers and users
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-gray-800">
            Admin Dashboard — Organization Control Center
          </h1>
          <p className="text-xs text-gray-500 mt-1">
            Welcome {user?.name}. You have full organization permissions to manage teams and assign tasks to managers and users.
          </p>
        </div>
        ...
        >
          {showAddForm ? "Close Form" : "+ Assign Task to Manager/User"}
        </button>
      </div>

      {/* Task Status Overview Across Organization */}
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
        ...
        <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-xs">
          <p className="text-xs text-purple-600 font-medium">Total Teams</p>
          <p className="text-2xl font-bold text-purple-700 mt-1">{teams.length}</p>
        </div>
      </div>
      ...
      {showAddForm && (
        <form onSubmit={handleCreateTask} className="bg-white p-5 rounded-xl border border-blue-200 shadow-sm space-y-4">
          <h3 className="text-base font-bold text-gray-800 border-b pb-2">
            Assign Task to Manager or User
          </h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="e.g. Database Migration to Production"
